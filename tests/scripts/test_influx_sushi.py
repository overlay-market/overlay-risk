...
class TestInfluxSushi(unittest.TestCase):
    ...
    def test_get_quotes(self):
        ...
        of quote dicts to use in fetching from SushiSwap.
    ...
    def test_get_prices(self):
        """
        get_prices(q) should fetch on-chain data (timestamp, priceCumulative0, priceCumulative1) from SushiSwap V2 `pool`
        address for a quote object `q`

        Should return a pandas DataFrame with columns ['timestamp', 'priceCumulative0', 'priceCumulative1']
        """
        pass

    def test_main(self):
        """
        main() should fetch current (timestamp, priceCumulative0, priceCumulative1) values from SushiSwap for
