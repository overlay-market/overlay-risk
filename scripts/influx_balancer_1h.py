...
def get_config() -> tp.Dict:
    return {
        ...
        "bucket": os.getenv('INFLUXDB_BUCKET', 'ovl_bal_1h'),
...
def get_point_settings() -> PointSettings:
    ...
    point_settings.add_default_tag("influx-bal", "ingest-data-frame")
    return point_settings


def get_bal_abi_path() -> str:
    base = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base, 'constants/balancer.abi.json')


def get_bal_abi() -> tp.Dict:
    with open(get_bal_abi_path()) as f:
        data = json.load(f)
    return data
...
def POOL(addr: str, abi) -> Contract:
    ...
    return Contract.from_abi("balancerv2", addr, abi)
...
def read_cumulatives(args: tp.Tuple) -> tp.Tuple:
    ...
    (pair, t_at) = args
    (b, b_t) = get_b_t(t_at)
    (twap) = pair.getTimeWeightedAverage(
                                        [(0, 3600, 0)],
                                        block_identifier=b
                                        )
    return (b_t, twap)
...
def get_bal_cumulatives(quotes, query_api, config, t_end):
    abi = get_bal_abi()
    t_step = 500
...
def main():
    ...
    while True:
        get_bal_cumulatives(quotes, query_api, config, t_end)
